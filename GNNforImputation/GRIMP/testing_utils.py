    ...
    for idx_sample, sample in enumerate(graph_dataset.test_positive_samples):
        ...
        if graph_dataset.graph_name == 'nn':
            ...
            if graph_dataset.graph_name == 'edge':
                target_row, target_col, target_val = graph_dataset.test_positive_triplets[idx_sample]
        if graph_dataset.graph_name == 'nn':
        ...
        if graph_dataset.graph_name != 'multilabel':
            ...
            imputed_cell_value = graph_dataset.idx2val[chosen_value + graph_dataset.num_row_col_nodes]
...
def generate_imputed_dataset_multilabel(graph_dataset, gnn_model, link_predictor):
    ...
    df_imputed = graph_dataset.df_missing.copy(deep=True)
    df_orig = graph_dataset.df_orig.copy(deep=True)
    results = np.zeros(shape=(graph_dataset.test_positive_samples.shape[0], len(graph_dataset.distinct_value_set)))
    for idx_sample, sample in enumerate(graph_dataset.test_positive_samples):
        test_vector = graph_dataset.test_positive_tuples[idx_sample].unsqueeze(0)
        triplet_vector = graph_dataset.test_positive_triplets[idx_sample]
        imputed_row, imputed_col, imputed_cell = tuple(map(torch.Tensor.item, triplet_vector))
        # tuple_sample = graph_dataset.test_positive_tuples[idx_sample]
        result = link_predictor(h, test_vector).reshape(-1, 1)
...
def generate_imputed_dataset_multitask(graph_dataset, best_state, init_params, skip_gnn):
    ...
    result = multitask_model(h, graph_dataset.test_positive_samples, graph_dataset.boundaries_test, step='test')
    ...
    for col_idx, col_imputations in result.items():
        ...
        for idx, triplet_vector in enumerate(iter_test_triplets):
            true_row_idx, true_col_idx, true_cell_idx = tuple(map(torch.Tensor.item, triplet_vector))
            if col_idx == true_col_idx:
                if col in graph_dataset.numerical_columns:
                    imputed_cell_value = choices[idx].item()
                else:
                    imputed_cell_value = graph_dataset.idx2val[graph_dataset.attribute_domain_cell_id_dict[true_col_idx][choices[idx].item()]]
...
def measure_imp_accuracy(graph_dataset, df_imp, logger=None):
        ...
        if df_dirty[col].isna().any():
            ...
            if col in graph_dataset.numerical_columns:
                ...
                try:
                    ...
                    print(f'Column {col} was converted to float before measuring accuracy.')
                except ValueError:
                    pass
    ...
    tot_true = sum([score for col, score in true_dict.items() if acc_type[col] == 'Accuracy'])
    tot_missing = sum([score for col, score in missing_dict.items() if acc_type[col] == 'Accuracy'])

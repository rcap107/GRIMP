...
def parse_args():
    ...
    parser.add_argument(
        "--quantiles", type=int, default=[10], nargs="*", action="store"
    )
...
def return_cut_datasets(df_missing, df_orig, cut_cols, quantiles):
    ...
    for idx, col in enumerate(cut_cols):
        df_missing[col], bins[col] = pd.qcut(
            df_missing[col], quantiles[idx], retbins=True, duplicates="drop"
        )
...
if __name__ == "__main__":
    ...
    df_missing, df_orig = return_cut_datasets(
        df_missing, df_orig, cut_cols=cut_columns, quantiles=quantiles
    )

    m = {args.df_missing: df_missing, args.df_orig: df_orig}

...
def prepare_labels(graph_dataset: ImputationTripartiteGraphMultilabelClassifier):
    ...
    labels[graph_dataset.num_rows : graph_dataset.num_row_col_nodes] = torch.ones(
        size=(graph_dataset.num_columns, 1)
    )
    labels[graph_dataset.num_row_col_nodes :] = 2 * torch.ones(
        size=(graph_dataset.num_total_nodes - graph_dataset.num_row_col_nodes, 1)
    )
...
def train(g: ImputationTripartiteGraphMultilabelClassifier, model, predictor):
    ...
    for e in range(200):
        ...
        if e % 5 == 0:
            print(
                "In epoch {}, loss: {:.3f},  (best {:.3f}),  (best {:.3f})".format(
...
if __name__ == "__main__":
    ...
    plot_PCA(
        graph_dataset.graph.ndata["features"].detach().numpy(),
        labels=prepare_labels(graph_dataset),
        title="Starting features",
    )

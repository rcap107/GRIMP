...
class ImputationTripartiteGraphMultilabelClassifier(DGLDataset):
    ...
    def _prepare_node_dictionaries(self):
        ...
        self.col2idx = dict(
            (col, self.num_rows + i) for (i, col) in enumerate(self.df_missing.columns)
        )
        self.idx2col = dict(
            (self.num_rows + i, col) for (i, col) in enumerate(self.df_missing.columns)
        )
        ...
        self.cell_to_dict = {
            self.df_missing.iloc[row, col]: col
    ...
    def load_external_features(self):
        ...
        self.graph.ndata["features"] = torch.zeros(
            size=(self.num_total_nodes, self.num_features)
        )
        temp_features = torch.zeros(
            size=(self.num_total_nodes, self.ext_features.shape[1])
        )
            ...
            for rhs in self.fd_dict:
                ...
                for fd in fds_subset:
                    ...
                    if not self.only_rhs:
                        ...
                        new_edges += [
                            (l1, l2) for l1 in lhs_idxs for l2 in lhs_idxs if l1 != l2
                        ]
    ...
    def add_inrow_edges(self):
        ...
        for idx, row in self.df_missing.iterrows():
            tmp_edges = [
                ...
                for l1 in row
                for l2 in row
                if l1 != l2 and (l1 is not np.nan and l2 is not np.nan)
            ]
    ...
    def compute_node_features(self):
        ...
        self.graph.ndata["features"] = torch.zeros(
            (self.num_total_nodes, self.num_features)
        )
        ...
        self.graph.nodes[range(self.num_rows, self.num_rows + self.num_columns)].data[
            "features"
        ] = torch.eye(self.num_columns).repeat_interleave(scaling, 1)

        # Features for cell nodes: each node is connected to each column
        self.graph.nodes[range(self.num_row_col_nodes, self.flag_index)].data[
            "features"
        ] = torch.ones(
            self.num_total_nodes - (self.num_row_col_nodes + 1), self.num_features
        )
        ...
        if self.random_init:
            ...
            self.graph.nodes[range(0, self.num_total_nodes)].data[
                "features"
            ] *= torch.FloatTensor(self.num_total_nodes, self.num_features).uniform_()
    ...
    def generate_labels_multitask(self, triplets):
        labels = {
            col_id: [] for col_id, val in self.attribute_domain_cell_id_dict.items()
        }
        weights = {
            col_id: [] for col_id, val in self.attribute_domain_cell_id_dict.items()
        }
        ...
        weights_t = [
            torch.FloatTensor(weights[col]) for col in weights if len(weights[col]) > 0
        ]
    ...
    def get_fd_context(self, v, col):
        if col in self.fd_dict:
            ...
            fd_tuple = [
                val if idx in flat_fds else self.flag_index for idx, val in enumerate(v)
            ]
        else:
    ...
    def process(self):
        ...
        if self.architecture == "multitask":
            self.labels, self.weights = self.generate_labels_multitask(
                self.train_positive_triplets
            )
            self.labels_valid, self.weights_valid = self.generate_labels_multitask(
                self.valid_positive_triplets
            )

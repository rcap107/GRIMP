...
def get_q_k(k_strat, graph_dataset: ImputationTripartiteGraphMultilabelClassifier):
    ...
    fill = [_ if _ not in graph_dataset.flat_fds else [_]+graph_dataset.flat_fds[_] for _ in range(graph_dataset.num_columns)]

    if k_strat == 'full':
        # Case 1: full diagonal
        k = torch.eye(graph_dataset.num_columns).repeat(((graph_dataset.num_columns, 1, 1)))
    elif k_strat == 'single':
        # Case 2: Only column t active in slice t, FDs have value 0.5
        k = torch.zeros((graph_dataset.num_columns, graph_dataset.num_columns, graph_dataset.num_columns))
...
class MultiTaskPredictor(nn.Module):
    def __init__(self, shared_in_feats, shared_out_feats, shared_h_feats=32, shared_h_layers=2,
                head_h_layers=2, head_out_feats_list=[], shared_out_feats_list=None, input_tuple_length=None,
                 dropout=0, batchnorm=False, shared_model='linear', head_model='attention', graph_dataset=None,
                 k_strat='single', no_relu=False, no_sm=False,
                 device='cpu'):
        '''
        ...
        if self.shared_attention:
            ...
            if input_tuple_length is not None:
                self.base_layer = SharedLayer(shared_in_feats, shared_h_feats, shared_out_feats, shared_h_layers, dropout,
                                              input_tuple_length=input_tuple_length, device=device
                                              )
            else:
                self.base_layer = SharedLayer(shared_in_feats, shared_h_feats, shared_out_feats, shared_h_layers, dropout,
                                              device)
        ...
        if self.head_attention:
            if len(head_out_feats_list) > 0:
                ...
                for idx, feats in enumerate(head_out_feats_list):
                    if feats is None:
                        ...
                        continue
                    new_module = AttentionHead(q, k[idx].clone().detach(), None,dropout=dropout, out_feat_size=self.in_feats, head_out=feats,
                                               no_relu=no_relu, no_sm=no_sm,
                                               device=device)
            ...
            if len(head_out_feats_list) > 0:
                ...
                for idx, feats in enumerate(head_out_feats_list):
                    if feats is None:
                        continue
                    new_module = AttributeLayer(self.out_feats, h_feats=self.h_feats, num_hidden_layers=head_h_layers,
                                                out_feats=feats, dropout=dropout, batchnorm=batchnorm, device=device)
                    self.heads.append(new_module)
                self.num_heads = len(self.heads)
            else:
                self.heads = None
                ...
                if shared_out_feats_list is None:
                    raise ValueError(f'Size of output layer required if no heads are present. ')
    ...
    def get_model_structure(self):
        ...
        if self.head_attention:
            ...
            struct_string += f'{len(self.heads)}xHeadLinear_x{self.head_h_layers}_{self.h_feats}'
...
class AttentionLayer(nn.Module):
    def __init__(self, q, k, input_feat_size, dropout=0.2, out_feat_size=None, head_out=None, hidden_feat_size=32, device='cpu'):
        ...
        self.m = nn.Parameter(torch.ones(size=(1, self.hidden_size),dtype=torch.float32, device=device),
                              requires_grad=True)
        # self.m = nn.Parameter(torch.ones(size=(1, self.q.shape[0]),dtype=torch.float32, device=device),
        #                       requires_grad=False)

        self.Wq = nn.Linear(self.q.shape[1], self.hidden_size, bias=True, )
...
class AttentionHead(nn.Module):
    def __init__(self, q, k, input_feat_size, dropout=0.2, out_feat_size=None, head_out=None, hidden_feat_size=32,
                no_relu=True, no_sm=True,
                 device='cpu'):
        ...
        self.m = nn.Parameter(torch.ones((1, self.hidden_size), dtype=torch.float32), requires_grad=False)
...
class SharedLayer(nn.Module):
    def __init__(self, shared_input_feats, h_feats, out_feats, num_hidden_layers=2, dropout=0, input_tuple_length=None, device='cpu'):
        super().__init__()
    ...
    def compute_triplet_matrix(self, h, triplets):
        ...
        self.triplet_embedding_matrix = h[[triplets]].reshape(len(triplets), self.shared_input_feats)
...
class AttributeLayer(nn.Module):
    def __init__(self, in_feats, h_feats, out_feats, num_hidden_layers=1, dropout=0, batchnorm=False, device='cpu'):
        super().__init__()
    ...
    def compute_triplet_matrix(self, h, triplets):
        ...
        self.triplet_embedding_matrix = h[[triplets]].reshape(len(triplets), self.attr_in_feats)

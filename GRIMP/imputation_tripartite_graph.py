...
class ImputationTripartiteGraph(DGLDataset):
    def __init__(self, original_data_file_name, missing_value_file_name, random_init=False,
                 node_mapping=None, ext_features=None, positive_negative_train_scale_factor=20):
        self.graph_name = ''
    ...
    def load_and_compute_stats(self):
        ...
        self.col2idx = dict((col, self.num_rows + i) for (i, col) in enumerate(self.df_orig.columns))
        self.idx2col = dict((self.num_rows + i, col) for (i, col) in enumerate(self.df_orig.columns))
...
class ImputationTripartiteGraphTripletPrediction(ImputationTripartiteGraph):
    def __init__(self, original_data_file_name, missing_value_file_name, random_init=False,
                 node_mapping=None, ext_features=None, compare_edgepred=True, pos_neg_scale_factor=20):
        if compare_edgepred:
        ...
        super().__init__(original_data_file_name, missing_value_file_name, random_init,
                         node_mapping, ext_features, pos_neg_scale_factor)
    ...
    def get_replacement(self, step, cell_node_id, col):
        ...
        found = looped = False
        start_idx = self.budget[tgt_col].loc[self.budget[tgt_col][tgt_col] == tgt_val].index[0]
    ...
    def get_positive_negative_triplets_for_training(self):
        ...
        self.pos_edges_to_remove = torch.zeros(self.num_non_missing_values*2,2).to(int)
        for row in range(self.num_rows):
            for col in range(self.num_columns):
                tgt_col = self.df_orig.columns[col]
                ...
                if pd.isnull(self.df_missing.iloc[row, col]) == False:
                    ...
                    self.train_positive_samples[self.positive_triplet_index] = (row, col_node_id, cell_node_id)
                    self.pos_edges_to_remove[2*self.positive_triplet_index,:] = torch.IntTensor([row, cell_node_id])
                    self.pos_edges_to_remove[2*self.positive_triplet_index+1,:] = torch.IntTensor([cell_node_id, col_node_id])
                    ...
                    for index2 in range(self.positive_negative_train_scale_factor):
                        replacement_cell_id = self.get_replacement(index2, cell_node_id, col)
                        #
                        # replacement_cell_id = random.sample(attribute_domain, k=1)[0]
                        self.train_negative_samples[self.negative_triplet_index] = (row, col_node_id, replacement_cell_id)
        ...
        self.train_positive_samples = torch.LongTensor(self.train_positive_samples[:self.positive_triplet_index])
        self.train_negative_samples = torch.LongTensor(self.train_negative_samples[:self.negative_triplet_index])
        ...
        for row in range(self.num_rows):
            for col in range(self.num_columns):
                if pd.isnull(self.df_missing.iloc[row, col]) == True:
                    ...
                    self.test_positive_samples[positive_triplet_index] = (row, col_node_id, cell_node_id)
                    self.pos_edges_to_remove[2*positive_triplet_index,:] = torch.IntTensor([row, cell_node_id])
                    self.pos_edges_to_remove[2*positive_triplet_index+1,:] = torch.IntTensor([cell_node_id, col_node_id])
                    ...
                    for cell_node_id in attribute_domain:
                        self.test_negative_samples[negative_triplet_index] = (row, col_node_id, cell_node_id)
    ...
    def create_train_positive_negative_graphs(self):
        ...
        print("Size of training positive and negative pos_triplets ", self.train_positive_triplets_size, self.train_negative_triplets_size)
    ...
    def create_test_positive_negative_graphs(self):
        ...
        print("Size of testing positive and negative pos_triplets ", self.test_positive_samples_size, self.test_negative_samples_size)
        self.get_positive_negative_triplets_for_testing()


class ImputationTripartiteGraphEdgePrediction(ImputationTripartiteGraph):
    def __init__(self,original_data_file_name, missing_value_file_name, random_init,
                 node_mapping, ext_features, pos_neg_scale_factor):
        super(ImputationTripartiteGraphEdgePrediction, self).__init__(original_data_file_name, missing_value_file_name, random_init,
                                                                      node_mapping, ext_features, pos_neg_scale_factor)
    ...
    def get_negative_edges_for_training(self):
        ...
        random_cell_ids = torch.randint(low=cell_id_start, high=cell_id_end, size=(self.train_negative_triplets_size,))
        random_row_ids = torch.randint(low=row_id_start, high=row_id_end, size=(self.train_negative_triplets_size,))
        ...
        for row in range(self.num_rows):
            for col in range(self.num_columns):
                if pd.isnull(self.df_missing.iloc[row, col]) == True:
                    ...
                    # self.test_pos_neg_matches[positive_triplet_index] = []
                    self.test_positive_triplets[positive_triplet_index] = torch.IntTensor((row, col_node_id, cell_node_id))
    ...
    def create_test_positive_negative_graphs(self):
        ...
        start_pos, end_pos, start_neg, end_neg = self.get_positive_negative_edges_for_testing()
...
class ImputationTripartiteGraphNearestNeighbor(ImputationTripartiteGraphTripletPrediction):
    def __init__(self, original_data_file_name, missing_value_file_name, random_init=False,
                 node_mapping=None, ext_features=None, pos_neg_scale_factor=20, with_cid=False):
        self.with_cid = with_cid
        super().__init__(original_data_file_name, missing_value_file_name, random_init,
                         node_mapping, ext_features, False,pos_neg_scale_factor)
        self.graph_name = 'nn'
    ...
    def create_train_positive_negative_graphs(self):
        ...
        self.train_negative_samples_size = self.train_positive_samples_size*self.positive_negative_train_scale_factor

        self.train_positive_samples = np.zeros((self.train_positive_samples_size, self.input_tuple_length))
        self.train_negative_samples = np.zeros((self.train_negative_samples_size, self.input_tuple_length))
        print("Size of training positive pos_triplets ", self.train_positive_samples_size)
        print("Size of training negative pos_triplets ", self.train_negative_samples_size)
    ...
    def get_positive_negative_triplets_for_testing(self):
        super(ImputationTripartiteGraphNearestNeighbor, self).get_positive_negative_triplets_for_testing()
        self.test_positive_tuples = torch.IntTensor(self.test_positive_samples_size, self.input_tuple_length)
        self.test_negative_tuples = torch.IntTensor(self.test_negative_samples_size, self.input_tuple_length)
...
class ImputationTripartiteGraphAverageTuple(ImputationTripartiteGraphTripletPrediction):
    ...
    def get_positive_negative_samples_for_training(self):
        ...
        for row_num, row in self.df_dropped.iterrows():
            ...
            for col_num, col in enumerate(self.df_dropped.columns):
                ...
                self.train_positive_samples[positive_samples, 1:] = [self.val2idx[v] for v in full_tuple]
                self.train_positive_triplets[positive_triplets, : ] = (row_num, col_id, val_id)

                self.train_negative_samples[positive_samples, 1:] = [self.val2idx[v] for v in full_tuple]
                attribute_domain = self.attribute_domain_cell_id_dict[col_id][:]
                attribute_domain.remove(val_id)
                random.shuffle(attribute_domain)
                for attr_val in attribute_domain[:self.positive_negative_train_scale_factor]:
                    self.train_negative_triplets[negative_triplets, : ] = (row_num, col_id, attr_val)
                    negative_triplets += 1
                positive_triplets += 1
                positive_samples += 1
                negative_samples += 1
    ...
    def get_positive_negative_triplets_for_testing(self):
        super(ImputationTripartiteGraphAverageTuple, self).get_positive_negative_triplets_for_testing()
        ...
        for sample in self.test_positive_samples:
            ...
            self.test_positive_tuples[positive_samples, 1:] = torch.IntTensor([self.val2idx[v] for v in full_tuple])
            ...
            for domain_value in range(len(attribute_domain)):
                ...
                if inner_val_id != val_id:
                    ...
                    self.test_negative_triplets[negative_samples, :] = torch.IntTensor((row_num, col_id, inner_val_id))
...
class ImputationTripartiteGraphMultilabelClassifier(ImputationTripartiteGraphAverageTuple):
    def __init__(self, original_data_file_name, missing_value_file_name, random_init=False,
                 node_mapping=None, ext_features=None, pos_neg_scale_factor=20, with_cid=False):
        self.with_cid = with_cid
        super().__init__(original_data_file_name, missing_value_file_name, random_init,
    ...
    def create_train_positive_negative_graphs(self):
        ...
        self.df_dropped = self.df_missing.dropna(axis=0).copy().reset_index(drop=True)
        self.train_positive_samples_size = len(self.df_dropped)*len(self.target_columns)
        self.train_negative_samples_size = self.train_positive_samples_size*self.positive_negative_train_scale_factor

        self.train_positive_samples = np.zeros((self.train_positive_samples_size, self.input_tuple_length))
        self.train_negative_samples = np.zeros((self.train_negative_samples_size, self.input_tuple_length))

        self.train_positive_triplets_size = len(self.df_dropped)*len(self.target_columns)
        self.train_negative_triplets_size = self.train_positive_triplets_size * self.positive_negative_train_scale_factor
    ...
    def create_test_positive_negative_graphs(self):
        super(ImputationTripartiteGraphMultilabelClassifier, self).create_test_positive_negative_graphs()
    ...
    def get_positive_negative_samples_for_training(self):
        ...
        for row_num, row in self.df_dropped.iterrows():
            ...
            for col in self.target_columns:
                ...
                self.train_positive_samples[positive_samples, :] = [self.val2idx[v] for v in full_tuple]
                self.train_positive_triplets[positive_triplets, : ] = (row_num, col_id, val_id)
    ...
    def get_positive_negative_triplets_for_testing(self):
        ...
        for sample in self.test_positive_samples:
            ...
            full_tuple = self.df_orig.iloc[row_num].tolist()
            full_tuple.remove(self.idx2val[val_id])
            self.test_positive_tuples[positive_samples, :] = torch.IntTensor([self.val2idx[v] for v in full_tuple])
            self.test_positive_triplets[positive_samples, :] = torch.IntTensor((row_num, col_id, val_id))
            # for col_num, col in enumerate(self.df_orig.columns):
            #     val = self.df_orig.iloc[positive_samples, col_num]
            #     inner_val_id = self.val2idx[val]

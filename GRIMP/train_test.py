...
def train(graph_dataset, gnn_model, link_predictor, node_predictor=None, epochs=100, device='cpu', logger=None):
    optimizer = torch.optim.Adam(itertools.chain(gnn_model.parameters(), link_predictor.parameters()), lr=0.01)
    optimizer2 = torch.optim.Adam(itertools.chain(gnn_model.parameters(), link_predictor.parameters()), lr=0.01)
    ...
    # for epoch in tqdm(range(epochs)):
        ...
        if graph_dataset.graph_name == 'edge':
            ...
            loss = compute_loss(pos_score.squeeze(1), neg_score.squeeze(1), device=device)
            # loss = compute_hinge_loss(pos_score.squeeze(1), neg_score.squeeze(1), device=device)
        elif graph_dataset.graph_name == 'avg':
            pos_score = link_predictor(h, graph_dataset.train_positive_samples, graph_dataset.train_positive_triplets)

            neg_score = link_predictor(h, graph_dataset.train_negative_samples, graph_dataset.train_negative_triplets)
            ...
            # loss = compute_loss_cosine(pos_score, graph_dataset, h)
            loss = compute_loss_multilabel(pos_score.squeeze(1), graph_dataset, device=device)
            # loss = compute_hinge_loss(pos_score.squeeze(1), neg_score.squeeze(1), device=device)
